...
AddListener("class_change", 0, function (player, params)
{
    ...
    if (IsRoundSetup() && IsValidPlayer(player) && !IsValidBoss(player))
    {
        ...
        //Only actually kill them the first time - if they're already dead, repeated
        //class swaps would otherwise spam TakeDamage/kill events with no visible respawn.
        local isAlive = GetPropInt(player, "m_lifeState") == LIFE_STATE.ALIVE;
        if (isAlive)
        {
            player.TakeDamage(9999, 0, null);
            SetPropInt(player, "m_lifeState", LIFE_STATE.DEAD);
